...
public class SquareVertsRenderer : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        drawBuffer = new ComputeBuffer(positions.Length * 16, DRAW_STRIDE, ComputeBufferType.Append);
        ...
        idMeshGenKernel = meshGenComputeShader.FindKernel("Cube");
        idTriToVertKernel = triToVertComputeShader.FindKernel("Main");

        // Set data on the shaders
        meshGenComputeShader.SetBuffer(idMeshGenKernel, "_SourceVertices", sourceVertBuffer);
        meshGenComputeShader.SetBuffer(idMeshGenKernel, "_SourceTriangles", sourceTriBuffer);
        meshGenComputeShader.SetBuffer(idMeshGenKernel, "_DrawTriangles", drawBuffer);
        meshGenComputeShader.SetInt("_NumSourceTriangles", numTriangles);
    ...
    private void LateUpdate()
    {
        ...
        //meshGenComputeShader.SetFloat("_PyramidHeight", pyramidHeight * Mathf.Sin(animationFrequency * Time.timeSinceLevelLoad));
        //meshGenComputeShader.SetFloat("_PyramidHeight", pyramidHeight);
        ...
        meshGenComputeShader.Dispatch(idMeshGenKernel, dispatchSize, 1, 1);
